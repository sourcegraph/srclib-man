...
ifeq ($(OS),Windows_NT)
	SRCLIB_MAN_EXE := .bin/srclib-man.exe
...
install: ${SRCLIB_MAN_EXE}

govendor:
	go get github.com/kardianos/govendor
	govendor sync

${SRCLIB_MAN_EXE}: $(shell /usr/bin/find . -type f -and -name '*.go' -not -path './vendor/*')
	go build -o ${SRCLIB_MAN_EXE}
